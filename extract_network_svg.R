...
#' @param tol Numeric or NULL. Maximum snap distance (in SVG user units)
#'   before an edge endpoint is flagged as suspect. If NULL (the default),
#'   the tolerance is computed automatically as 0.1 percent of the SVG's
#'   coordinate range.
#'
...
#'
#' @export
extract_svg_links <- function(svg_file, tol = NULL) {
  ...
  # Walk up from a node to the container, accumulating translate offsets.
  # This handles nodes nested inside cluster <g> elements that have their
  # own translate() transforms.
  cumulative_translate <- function(node, container) {
    offset  <- c(0, 0)
    current <- node
    while (!is.null(current)) {
      if (identical(current, container)) break
      tr <- parse_translate(xml_attr(current, "transform"))
      offset  <- offset + tr
  ...
  # Find the container <g> that holds both edges and nodes.
  # This is the parent of <g id="edges">.
  xp_edges_parent <- paste0("//s:g[@id=", q, "edges", q, "]")
  edges_g   <- xml_find_first(doc, xp_edges_parent, ns)
  container <- xml_parent(edges_g)
  ...
  ports <- rbindlist(lapply(node_groups, function(g) {
    ...
    tr         <- cumulative_translate(g, container)
  ...
  # Auto-compute tolerance if not provided: 0.1% of coordinate range
  if (is.null(tol)) {
    coord_range <- max(
      max(ports$x, na.rm = TRUE) - min(ports$x, na.rm = TRUE),
      max(ports$y, na.rm = TRUE) - min(ports$y, na.rm = TRUE)
    )
    tol <- coord_range * 0.001
    if (tol < 1) tol <- 1
  }
  ...
  max_from <- max(raw_edges$from_dist, na.rm = TRUE)
  max_to   <- max(raw_edges$to_dist, na.rm = TRUE)
  n_suspect <- sum(raw_edges$from_dist > tol | raw_edges$to_dist > tol)
  if (n_suspect > 0) {
    message(n_suspect, " edge(s) did not snap cleanly (dist > ",
            round(tol, 2), ", max dist = ", round(max(max_from, max_to), 2), ")")
  }
...
#' Diagnose edge snap distances for a single SVG
#'
#' Runs the extraction with no tolerance filtering and returns the raw
#' snap distances, plus summary statistics. Useful for understanding why
#' edges are flagged as suspect and choosing an appropriate tolerance.
#'
#' @param svg_file Character. Path to the SVG file.
#'
#' @return A list with:
#'   \describe{
#'     \item{summary}{Named vector: min, median, mean, p95, p99, max of
#'       all snap distances, plus the auto-computed tolerance.}
#'     \item{raw}{[data.table] with one row per edge: `edge_id`,
#'       `from_name`, `to_name`, `from_dist`, `to_dist`, `max_dist`.}
#'   }
#'
#' @examples
#' diag <- diagnose_snap("problem_file.svg")
#' diag$summary
#' hist(diag$raw$max_dist, breaks = 50)
#'
#' @export
diagnose_snap <- function(svg_file) {
  doc <- read_xml(svg_file)
  ns  <- c(s = "http://www.w3.org/2000/svg")
  q   <- intToUtf8(39L)
  ...
  cumulative_translate <- function(node, stop_node) {
    offset  <- c(0, 0)
    current <- node
    while (!is.null(current)) {
      if (identical(current, stop_node)) break
      tr <- parse_translate(xml_attr(current, "transform"))
      offset  <- offset + tr
      current <- xml_parent(current)
    }
  ...
  # Find the container (parent of edges group)
  xp_edges_parent <- paste0("//s:g[@id=", q, "edges", q, "]")
  edges_g   <- xml_find_first(doc, xp_edges_parent, ns)
  container <- xml_parent(edges_g)
  ...
  ports <- rbindlist(lapply(node_groups, function(g) {
    ...
    if (length(polys) == 0) return(NULL)
    pts <- as.numeric(strsplit(xml_attr(polys[[1]], "points"), "[, ]+")[[1]])
    xs <- pts[c(TRUE, FALSE)]; ys <- pts[c(FALSE, TRUE)]
    data.table(node_name = node_label(g), x = tr[1] + mean(xs), y = tr[2] + max(ys))
  }), fill = TRUE)
  ...
  raw_edges <- rbindlist(lapply(seq_along(edge_groups), function(i) {
    ...
    data.table(edge_id = i, start_x = start[1], start_y = start[2],
               end_x = end[1], end_y = end[2])
  }))
  ...
  raw_edges[, c("from_name", "from_dist") := nearest(start_x, start_y), by = edge_id]
  raw_edges[, c("to_name", "to_dist")     := nearest(end_x, end_y),     by = edge_id]
  raw_edges[, max_dist := pmax(from_dist, to_dist)]
  
  all_dists <- c(raw_edges$from_dist, raw_edges$to_dist)
  coord_range <- max(
    max(ports$x, na.rm = TRUE) - min(ports$x, na.rm = TRUE),
    max(ports$y, na.rm = TRUE) - min(ports$y, na.rm = TRUE)
  )
  auto_tol <- max(coord_range * 0.001, 1)
  
  summary_vec <- c(
    min       = round(min(all_dists), 4),
    median    = round(median(all_dists), 4),
    mean      = round(mean(all_dists), 4),
    p95       = round(quantile(all_dists, 0.95), 4),
    p99       = round(quantile(all_dists, 0.99), 4),
    max       = round(max(all_dists), 4),
    auto_tol  = round(auto_tol, 4),
    n_edges   = nrow(raw_edges),
    n_ports   = nrow(ports),
    coord_range = round(coord_range, 2)
  )
  
  message("Coord range: ", round(coord_range, 1),
          "  Auto tol: ", round(auto_tol, 2),
          "  Median dist: ", round(median(all_dists), 4),
          "  Max dist: ", round(max(all_dists), 2))
  
  list(
    summary = summary_vec,
    raw = raw_edges[, c("edge_id", "from_name", "to_name",
                        "from_dist", "to_dist", "max_dist"), with = FALSE]
  )
}
...
#' @param tol Numeric or NULL. Snap tolerance passed through to
#'   [extract_svg_links()]. Default NULL (auto-computed per SVG).
#'   Default 5.
#'
...
#'
#' @export
extract_national_network <- function(root, tol = NULL) {
...
#' @param tol Numeric or NULL. Snap tolerance passed to [extract_svg_links()].
#'   Default NULL (auto-computed per SVG).
#'   Default 5.
#'
...
#'
#' @export
network_diff <- function(svg_old, svg_new, tol = NULL) {
